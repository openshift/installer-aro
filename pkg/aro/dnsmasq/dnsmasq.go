...
{{- range $ProxyDomain := .ProxyDomains }}
address=/{{ $ProxyDomain }}/{{ .PrivateEndpointIP }}
{{- end }}
user=dnsmasq
group=dnsmasq
no-hosts
...
func config(clusterDomain, apiIntIP, ingressIP string, proxyDomains []string, privateEndpointIP string) ([]byte, error) {
	buf := &bytes.Buffer{}
...
func Ignition2Config(clusterDomain, apiIntIP, ingressIP string, proxyDomains []string, privateEndpointIP string) (*ign2types.Config, error) {
	config, err := config(clusterDomain, apiIntIP, ingressIP, proxyDomains, privateEndpointIP)
	if err != nil {
		return nil, err
	}
...
func Ignition3Config(clusterDomain, apiIntIP, ingressIP string, proxyDomains []string, privateEndpointIP string) (*ign3types.Config, error) {
	config, err := config(clusterDomain, apiIntIP, ingressIP, proxyDomains, privateEndpointIP)
	if err != nil {
		return nil, err
	}
...
func MachineConfig(clusterDomain, apiIntIP, ingressIP, role string, proxyDomains []string, privateEndpointIP string) (*mcfgv1.MachineConfig, error) {
	ignConfig, err := Ignition2Config(clusterDomain, apiIntIP, ingressIP, proxyDomains, privateEndpointIP)
	if err != nil {
		return nil, err
	}
