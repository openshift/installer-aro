...
func (cpc *CloudProviderConfig) Generate(dependencies asset.Parents) error {
	...
	case azuretypes.Name:
		...
		creds := installConfig.Azure.GetPlatformCredentials()
	...
	if installConfig.Config.Azure.ARO {
		for _, f := range []struct {
			filename string
			data     func(*installconfig.InstallConfig) ([]byte, error)
		}{
			{
...
func aroRole(*installconfig.InstallConfig) ([]byte, error) {
...
func aroRoleBinding(*installconfig.InstallConfig) ([]byte, error) {
...
func aroSecret(_installConfig *installconfig.InstallConfig) ([]byte, error) {
	creds := _installConfig.Azure.GetPlatformCredentials()
